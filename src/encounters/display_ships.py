...
def plot_vessels_on_map(gdf, distance, specificIndexesToPlot = None):
    ...
    for index, row in latest_locations.iterrows():
        folium.Marker(location=[row['Latitude'], row['Longitude']], popup=f"MMSI: {row['MMSI']}", icon=folium.Icon(color='red')).add_to(m)
...
def drawLineWithDistanceBetweenPoints(gdf, map_object):
    for i in range(len(gdf)):
        for j in range(i + 1, len(gdf)):
            ...
            folium.PolyLine(locations=[(lat1, lon1), (lat2, lon2)], color='green').add_to(map_object)
...
def drawSafeRadiusForVessel(gdf, map_object, distance):
    for i in range(len(gdf)):
        ...
        folium.Circle(location=(lat, lon), radius=(distance*1000), color='red', fill=True).add_to(map_object)
