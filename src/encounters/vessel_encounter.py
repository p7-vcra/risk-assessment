...
def vessel_encounters_server(
    data,
    active_pairs,
    distance_threshold=DISTANCE_THRESHOLD_IN_KM,
    temporal_threshold=TEMPORAL_THRESHOLD_IN_SECONDS,
):
    """
    ...
    timestamp = data["timestamp"].iloc[
        0
    ]  # Assuming all data chunks have the same timestamp
    ...
    if active_pairs.empty:  # Case 1: active_pairs is empty
        ...
        active_pairs, new_inactive_pairs = helper.update_pairs(
            timestamp, active_pairs, current_pairs, temporal_threshold
        )
        inactive_pairs = pd.concat([inactive_pairs, new_inactive_pairs])
        pairs_out = active_pairs[
            active_pairs["end_time"] - active_pairs["start_time"]
