...
def calculate_cri_vcra(
    ...
    return (
        ...
        np.nan,
        np.nan,
        max(0, min(1, vcra_model.predict(vcra_input)[0])),
    )
...
def calculate_cri(rec_own, rec_target, stationary_speed_threshold=1, vcra_model=None):
    ...
    own["course_rad"], target["course_rad"] = degrees_to_radians(
        own["course"]
    ), degrees_to_radians(target["course"])

    own["length_nmi"], target["length_nmi"] = (
        own["length"] / 1852,
        target["length"] / 1852,
    )
    ...
    (
        ...
        DCPA,
        TCPA,
    ) = cri.calc_cpa(own=own, target=target)

    if vcra_model is None:
        U_dcpa, U_tcpa, U_dist, U_bearing, U_speed, ves_cri = cri.calc_cri(
            own,
            target,
            dist_euclid,
            speed_r,
        ...
        U_dcpa, U_tcpa, U_dist, U_bearing, U_speed, ves_cri = calculate_cri_vcra(
            own,
            target,
            dist_euclid,
            rel_movement_direction,
...
def pairs_in_radius(df, diam=1000):
    ...
    df_kdtree = BallTree(df_rad, leaf_size=40, metric="haversine")
    ...
    point_neighbors, dist = df_kdtree.query_radius(
        df_rad, return_distance=True, sort_results=True, r=theta
    )
...
def get_key(timeslice, vessel_i, vessel_j, **kwargs):
    vessel_id_i, vessel_id_j = translate(timeslice, vessel_i, **kwargs), translate(
        timeslice, vessel_j, **kwargs
    )
    return tuple([vessel_id_i, vessel_id_j])


def current_pairs(
    dt, curr, coords=["lon", "lat"], diam=1852, stationary_speed_threshold=1, **kwargs
):
    if curr.empty:
    ...
    curr_pairs = itertools.chain.from_iterable(
        [get_pairs(pairs, dists) for pairs, dists in zip(pairs, dists)]
    )

    curr_pairs_mi = pd.DataFrame(
        data=[
            [
                get_key(curr, v_i, v_j, **kwargs),
                dist,
                dt,
                dt,
                [curr.iloc[v_i, coords_loc]],
                curr.iloc[v_i],
                curr.iloc[v_j],
            ]
            for dt, (v_i, v_j, dist) in zip(itertools.repeat(dt), curr_pairs)
            if curr.iloc[v_i].speed > stationary_speed_threshold
        ],
        columns=[
            "pair",
            "dist",
    ...
    # print(curr_pairs_mi.head())
    curr_pairs_mi = curr_pairs_mi[~curr_pairs_mi.index.duplicated(keep="first")]

    assert not curr_pairs_mi.index.duplicated().any(), ValueError(
        "[current_pairs] Violation of Primary Key Constraint..."
    )
...
def update_inactives(inactive_pairs, candidate_pairs, dt_thresh):
    ...
    return pd.concat(
        (inactive_pairs, candidate_pairs.loc[mask_duration]), ignore_index=False
    )
...
def curr_encounters(
    ...
    mask_distance = (
        alive_pairs.dist_prev >= alive_pairs.dist
    )  # ## Find monotonically decreasing (w.r.t distance) pairs
    ...
    active_alive_pairs.geometry = (
        active_alive_pairs.geometry_prev + active_alive_pairs.geometry
    )

    return active_alive_pairs.loc[:, active_idx_slice], inactive_alive_pairs


def encountering_vessels_timeslice(
    curr_pairs,
    inactive_pairs,
    dt_thresh,
    active_idx_slice=pd.IndexSlice["dist":],
    ...
    mask_inactive, disappeared_pairs = query_nans(
        curr_pairs, "dist", inactive_idx_slice
    )
...
def encountering_vessels(
    ...
    **kwargs,
    ...
    (active_pairs, inactive_pairs), results = (
        prior_pairs if prior_pairs is not None else (pd.DataFrame(), pd.DataFrame())
    ), []
    print(f"Prior: {len(active_pairs)=}, {len(inactive_pairs)=}, {len(results)=}")

    for dt, curr in tqdm.tqdm(data.groupby(dt_name)):
        curr_slice = current_pairs(
            dt,
            curr,
            oid_name=oid_name,
            coords=coords,
        ...
        active_pairs, inactive_pairs = encountering_vessels_timeslice(
            curr_pairs, inactive_pairs, dt_thresh
        )
        ...
        active_pairs_cri = (
            active_pairs.loc[active_pairs.end - active_pairs.start >= dt_thresh]
            .apply(
                lambda l: [
                    *get_kinematics(l.kinematics_own),
                    *get_kinematics(l.kinematics_target),
                    *calculate_cri(
                        l.kinematics_own,
                        l.kinematics_target,
                        stationary_speed_threshold=stationary_speed_threshold,
        ...
        results.append(
            pd.concat(
                {dt: pd.DataFrame(active_pairs_cri, columns=CRI_DATASET_FEATURES)},
                names=[
                    dt_name,
                ],
            )
