...
def calc_cri(
    ...
    crit_safe_dist, avoidance_measure_dist = calc_crit_dist(
        data["vessel_1_length"] / NMI_IN_KM * 1000, rel_bearing
    )  # Length is in meters, so we convert it to nmi
    u_dist = cpa_membership(euclidian_dist, crit_safe_dist, avoidance_measure_dist)

    u_bearing = rel_bearing_membership(rel_bearing)

    u_speed = speed_ratio_membership(
        data["vessel_1_speed"], data["vessel_2_speed"], rel_movement_direction
    )
...
def calc_crit_dist(own_length, rel_bearing):
    ...
    avoidance_measure_dist = 1.7 * np.cos(angle) + np.sqrt(
        4.4 + 2.89 * np.cos(angle) ** 2
    )
