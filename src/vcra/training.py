...
def evaluate_clf(clf, X, y, train_index, test_index, include_indices=False):
    logger.info(f'Training with {len(train_index)} samples; Testing with {len(test_index)} samples')
...
def train_mlp_vcra(data_dir, X_sub, y_sub, y_bin_sub, tag=''):
    ...
    mlp_vcra_features = ['vessel_1_speed', 'vessel_1_course', 'vessel_2_speed', 'vessel_2_course', 'euclidian_dist', 'azimuth_target_to_own', 'rel_movement_direction']
    ...
    mlp_vcra_skf_results_df.to_pickle(f'{data_dir}/mlp_vcra_skf_results_v14{tag}.pickle')
