...
class TestVesselPairsInRadius(unittest.TestCase):
    ...
    def test_get_vessel_pairs_in_radius_within_threshold(self):
        ...
        points, distances = helper.get_vessels_in_radius(
            self.vessel_data, distance_threshold_in_km
        )
        ...
        expected_distances = np.array(
            [
                [0.0, 1.039594270818829, 8.960918455466697],
                [0.0, 1.039594270818829, 7.9213246481542114],
                [0.0, 7.9213246481542114, 8.960918455466697],
            ]
        )  # Distances in km between points
        ...
        for distance_array, expected_array in zip(distances, expected_distances):
            np.testing.assert_array_almost_equal(
                distance_array, expected_array, decimal=2
            )
